...
public:
	...
	public:
		...
		bool operator==(const Position& q) const { return pQN == q.pQN; }//노드가 같은지 판별
		bool operator!=(const Position& q) const { return pQN != q.pQN; }//노드가 다른지 판별
		Element& operator*() const { return pQN->element(); }
		Position operator++()//반복자의 증감연산자
		{
			return Position(pQN->next);//endofbucket에서 사용
		}
...
public:
	...
	Position begin();//반복자 begin()
	Position last();//반복자 end() endofbucket 판별
	bool empty();//반복자 empty

};
template <typename Key, typename Element>
typename SkipList<Key, Element>::Position SkipList<Key, Element> ::begin()
{
	Position pos = start;

	while (pos.below() != Position(NULL))//level 0으로 이동
	{
		pos = pos.below();
	}
	pos = pos.after();//첫번째  
...
template <typename Key, typename Element>
typename SkipList<Key, Element>::Position SkipList<Key, Element>::last()
{
	Position pos = end;
...
template <typename Key, typename Element>
bool SkipList<Key, Element>::empty()
{
	return (num_elements == 0);//비어있는지 판별
}

template <typename Key, typename Element>
SkipList<Key, Element>::SkipList()
{
	height = 0;//높이 초기화
...
template <typename Key, typename Element>
typename SkipList<Key, Element>::Position SkipList<Key, Element>::SkipListSearch(Key& k)
{
...
template <typename Key, typename Element>
typename SkipList<Key, Element>::Position SkipList<Key, Element>::SkipListInsert(ostream& fout, Key k, const Element e)
{
	Position p, q, t;
...
template <typename Key, typename Element>
void SkipList<Key, Element>::SkipListNodeDelete(Position p)
{
...
template <typename Key, typename Element>
void SkipList<Key, Element>::SkipListRemove(ostream& fout, Key k)
{
...
template <typename Key, typename Element>
void SkipList<Key, Element>::PrintSkipList(ostream& fout)
{
...
template <typename Key, typename Element>
typename SkipList<Key, Element>::Position SkipList<Key, Element>::insertAfterAbove(ostream& fout, Position p, Position q, const Key k, const Element e)
{
