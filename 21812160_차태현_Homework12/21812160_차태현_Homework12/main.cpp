...
*
*
*프로그램 작성자: 21812160 차태현(2021년 11월27일),
*최종 수정 및 보완: 2021년 11월 27일 (차태현)
*/
#include <iostream>
#include <fstream>
#include <list>
...
void main()
{
	...
	while (1)//단어가 완전히 일치할 경우 삭제하는동작
	{
		cout << "\nTesting trie_delete_key for keywords from trie data structure." << endl;
		cin >> keyStr;//단어 검색
		if (keyStr == string("."))//.이 들어올경우 break
			break;
		
		cout << "Trie-Deleting (" << keyStr << ") :" << endl;
		
		itr = predictVocas.begin();
		for (int i = 0; i < predictVocas.size(); i++)
		{	
			trie_myVoca.deleteKeyStr(keyStr);//해당키워드와 일치하는 단어삭제
			pVoca = *itr;
			++itr;	
			//trie_myVoca.fprintTrie(cout);//삭제된 단어가 트리에 있는지 확인차 실행
		}
	}
