...
public:
	...
	void fprint(ostream& cout, int elements_per_line);//출력
	void fprintSample(ostream& cout, int elements_per_line, int num_sample_lines);//샘플출력
...
template<typename K, typename V>
void TA_Entry<K, V>::fprintSample(ostream& fout, int elements_per_line, int num_sample_lines)
{
	int count = 0;
