...
public:
	...
	bool isFull() {return (this->end >= this->heapPriQ_capa); }//용량이랑 같을경우
	T_Entry<K, V>* insert(T_Entry<K, V>& elem);//원소 삽입
...
HeapPrioQueue<K, V>::HeapPrioQueue(int capa, string nm)//생성자
	:CompleteBinaryTree<K, V>(capa, nm),heapPriQ_capa(capa)//생성자 호출
{
